 ...
 * Constructor
 */
ImageReader::ImageReader(int32_t width, int32_t height, enum AIMAGE_FORMATS format)
    : reader_(nullptr), presentRotation_(0) {
  callback_ = nullptr;
  callbackCtx_ = nullptr;

  media_status_t status = AImageReader_new(width, height, format,
...
void ImageReader::RegisterCallback(void* ctx,std::function<void(void *ctx, ImageReader *reader)> func) {
callbackCtx_ = ctx;
callback_ = func;
}
