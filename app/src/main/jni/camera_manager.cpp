...
NDKCamera::NDKCamera(uint32_t camera_type)
        : cameraMgr_(nullptr),
          activeCameraId_(""),
          outputContainer_(nullptr),
          captureSessionState_(CaptureSessionState::MAX_STATE),
          cameraFacing_(camera_type/*ACAMERA_LENS_FACING_BACK*/),
    ...
    for (int i = 0; i < entry.count; i += 4) {
        ...
        if (format == AIMAGE_FORMAT_YUV_420_888 || format == AIMAGE_FORMAT_JPEG) {
            DisplayDimension res(entry.data.i32[i + 1],
                                 entry.data.i32[i + 2]);
            LOGE("%dx%d",disp.width(),disp.height());
...
void NDKCamera::EnumerateCamera() {
    ...
    //LOGI("%#x %s ",status, GetErrorStr(status));
    ...
    for (int i = 0; i < cameraIds->numCameras; ++i) {
        ...
        for (int tagIdx = 0; tagIdx < count; ++tagIdx) {
            if (ACAMERA_LENS_FACING == tags[tagIdx]) {
                ...
                if (cam.facing_ == cameraFacing_/*ACAMERA_LENS_FACING_BACK*/) {
